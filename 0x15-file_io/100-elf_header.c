#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
#include <elf.h>
#include <sys/types.h>
#include <sys/stat.h>

void if_elf(unsigned char *e_ident);
void _magic(unsigned char *e_ident);
void _Class(unsigned char *e_ident);
void _DATA(unsigned char *e_ident);
void _Version(unsigned char *e_ident);
void DisplayABI(unsigned char *e_ident);
void DisplayOsabi(unsigned char *e_ident);
void DisplayType(unsigned int e_type, unsigned char *e_ident);
void DisplayEntry(unsigned long int e_entry, unsigned char *e_ident);
void ExitElf(int elf);

/**
 * if_elf - (file is an ELF file?).
 * @e_ident: asd.
 */

void if_elf(unsigned char *e_ident)
{
	int index;

		for (index = 0; index < 4; index++)
	{
		if (e_ident[index] != 127 &&
		    e_ident[index] != 'E' &&
		    e_ident[index] != 'L' &&
		    e_ident[index] != 'F')
		{
			dprintf(STDERR_FILENO, "Error: Not an ELF file\n");
			exit(98);
		}
	}
}
/**
 * _DATA - wrhajnajaw.
 * @e_ident: awrhhrha.
 */

void _DATA(unsigned char *e_ident)
{
	printf("  Data:                              ");

	switch (e_ident[EI_DATA])
	{
	case ELFDATANONE:
		printf("none\n");
		break;
	case ELFDATA2LSB:
		printf("2's complement, little endian\n");
		break;
	case ELFDATA2MSB:
		printf("2's complement, big endian\n");
		break;
	default:
		printf("<unknown: %x>\n", e_ident[EI_CLASS]);
	}
}
/**
 * _magic - awe.
 * @e_ident: awer.
 */

void _magic(unsigned char *e_ident)
{
	int index;

	printf("  Magic:   ");

	for (index = 0; index < EI_NIDENT; index++)
	{
		printf("%02x", e_ident[index]);

		if (index == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
}

/**
 * _Class - wrhahwahwhawh.
 * @e_ident: awrhajwajrwjw.
 */

void _Class(unsigned char *e_ident)
{
	printf("  Class:                             ");

	switch (e_ident[EI_CLASS])
	{
	case ELFCLASSNONE:
		printf("none\n");
		break;
	case ELFCLASS32:
		printf("ELF32\n");
		break;
	case ELFCLASS64:
		printf("ELF64\n");
		break;
	default:
		printf("<unknown: %x>\n", e_ident[EI_CLASS]);
	}
}



/**
 * _Version - arwar.
 * @e_ident: dfydyryr.
 */

void _Version(unsigned char *e_ident)
{
	printf("  Version:                           %d",
	       e_ident[EI_VERSION]);

	switch (e_ident[EI_VERSION])
	{
	case EV_CURRENT:
		printf(" (current)\n");
		break;
	default:
		printf("\n");
		break;
	}
}
/**
 * DisplayType - fuu
 * @e_type: gyu
 * @e_ident: hj.
 */

void DisplayType(unsigned int e_type, unsigned char *e_ident)
{
	if (e_ident[EI_DATA] == ELFDATA2MSB)
		e_type >>= 8;

	printf("  Type:                              ");

	switch (e_type)
	{
	case ET_NONE:
		printf("NONE (None)\n");
		break;
	case ET_REL:
		printf("REL (Relocatable file)\n");
		break;
	case ET_EXEC:
		printf("EXEC (Executable file)\n");
		break;
	case ET_DYN:
		printf("DYN (Shared object file)\n");
		break;
	case ET_CORE:
		printf("CORE (Core file)\n");
		break;
	default:
		printf("<unknown: %x>\n", e_type);
	}
}

/**
 * DisplayOsabi - aweawetsd.
 * @e_ident: weawe
 */

void DisplayOsabi(unsigned char *e_ident)
{
	printf("  OS/ABI:                            ");

	switch (e_ident[EI_OSABI])
	{
	case ELFOSABI_NONE:
		printf("UNIX - System V\n");
		break;
	case ELFOSABI_HPUX:
		printf("UNIX - HP-UX\n");
		break;
	case ELFOSABI_NETBSD:
		printf("UNIX - NetBSD\n");
		break;
	case ELFOSABI_LINUX:
		printf("UNIX - Linux\n");
		break;
	case ELFOSABI_SOLARIS:
		printf("UNIX - Solaris\n");
		break;
	case ELFOSABI_IRIX:
		printf("UNIX - IRIX\n");
		break;
	case ELFOSABI_FREEBSD:
		printf("UNIX - FreeBSD\n");
		break;
	case ELFOSABI_TRU64:
		printf("UNIX - TRU64\n");
		break;
	case ELFOSABI_ARM:
		printf("ARM\n");
		break;
	case ELFOSABI_STANDALONE:
		printf("Standalone App\n");
		break;
	default:
		printf("<unknown: %x>\n", e_ident[EI_OSABI]);
	}
}

/**
 * DisplayABI - rttuy
 * @e_ident: tiy
 */

void DisplayABI(unsigned char *e_ident)
{
	printf("  ABI Version:                       %d\n",
	       e_ident[EI_ABIVERSION]);
}


/**
 * DisplayEntry - ery.
 * @e_entry: tyu.
 * @e_ident: rtu.
 */

void DisplayEntry(unsigned long int e_entry, unsigned char *e_ident)
{
	printf("  Entry point address:               ");

	if (e_ident[EI_DATA] == ELFDATA2MSB)
	{
		e_entry = ((e_entry << 8) & 0xFF00FF00) |
			  ((e_entry >> 8) & 0xFF00FF);
		e_entry = (e_entry << 16) | (e_entry >> 16);
	}

	if (e_ident[EI_CLASS] == ELFCLASS32)
		printf("%#x\n", (unsigned int)e_entry);

	else
		printf("%#lx\n", e_entry);
}

/**
 * ExitElf - asd.
 * @elf: sdfg.
 */

void ExitElf(int elf)
{
	if (close(elf) == -1)
	{
		dprintf(STDERR_FILENO,
			"Error: Can't close fd %d\n", elf);
		exit(98);
	}
}

/**
 * main - Entry point
 * @argc: argc.
 * @argv: argv.
 *
 * Return: 0 on success.
 */

int main(int __attribute__((__unused__)) argc, char *argv[])
{
	Elf64_Ehdr *Header1;
	int num1, num2;

	num1 = open(argv[1], O_RDONLY);
	if (num1 == -1)
	{
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
		exit(98);
	}
	Header1 = malloc(sizeof(Elf64_Ehdr));
	if (Header1 == NULL)
	{
		ExitElf(num1);
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
		exit(98);
	}
	num2 = read(num1, Header1, sizeof(Elf64_Ehdr));
	if (num2 == -1)
	{
		free(Header1);
		ExitElf(num1);
		dprintf(STDERR_FILENO, "Error: `%s`: No such file\n", argv[1]);
		exit(98);
	}

	if_elf(Header1->e_ident);
	printf("ELF Header1:\n");
	_magic(Header1->e_ident);
	_Class(Header1->e_ident);
	_DATA(Header1->e_ident);
	_Version(Header1->e_ident);
	DisplayOsabi(Header1->e_ident);
	DisplayABI(Header1->e_ident);
	DisplayType(Header1->e_type, Header1->e_ident);
	DisplayEntry(Header1->e_entry, Header1->e_ident);

	free(Header1);
	ExitElf(num1);
	return (0);
}

